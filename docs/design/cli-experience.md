...
完整路径可换行但不省略；相对路径明确相对当前工作目录。结束不重复日志路径，只给出实际存在的 report.md 位置。初始计划写成后同样显示一次该位置，生命周期见帮助与报告。
...
写入行分类、重新激活与 stale 分母按 [总体设计的统计定义](overview.md#section-8)处理；仅展示相关指标不改变统计规则。

## 进度与实现建议

展示阶段、已完成/计划块、耗时、ETA、HTTP 尝试速率与确认写入速率，单位明确。
日历准备显示 Preparing calendar，无数据 ETA；数据 ETA 使用最近 20 个完成逻辑块的耗时，至少 5 个样本且累计达到 10 秒后才显示；平均耗时包含请求、等待与写入。当前块超过窗口均值的 3 倍时暂不显示估计，重试等待、合并提交和停止阶段也不显示。

...
本文与请求规划算法、CLI 交互与输出设计共同构成设计 **v0.1.0-draft.12**，不单独递进页面版本。
...
每个测试按上表追溯规范，不在测试章节重新规定命令含义。实现验收要求这些检查通过并记录结果；真实接口验证单独执行第 16 章清单，验收平台为 WSL/Linux Python，覆盖 WSL 连接 Windows PostgreSQL；不要求 Windows 原生 Python。Excel 仅为可选 Demo。真实 Token 不进入自动测试，缺失集成数据库配置必须明确失败而非偷偷使用正式库。
...
当前设计为 v0.1.0-draft.12。后续每形成一轮完整修订，草案号递增为 draft.13、draft.14 等；同一轮零碎编辑不单独编号。三篇正文与设计入口同步更新版本，正式定版时移除预发布后缀，成为 v0.1.0。draft 与数字之间使用点分隔，按 SemVer 数字标识符排序。
...
| v0.1.0-draft.12 | 根据实际部署明确 WSL/Linux Python 验收范围，取消 Windows 原生 Python 要求；接入真实 GitHub 仓库与 Pages Actions 验证，补齐故障、矩阵和完整流程基准证据 |

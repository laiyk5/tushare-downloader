...
配置/参数校验 → 建立日志并显示路径 → 本地自检 → 块需求与交易日过滤 → 原子写入 report.md 的初始计划 → 顺序请求及写入 → 原子更新完整报告。--dry-run 复用自检与计划，只不请求远端、不修改数据库。计划只在内存存在。
...
按 10 MiB 轮转，本次所有分片保留；历史清理由用户维护。报告列出所有实际分片，启动文件路径不保证 tail 自动跟随其他分片，便捷跨片监看入口仍待设计，不能宣称已支持。
...
- [ ] 新旧文档链接、严格构建、Pages 部署通过；当前完整设计内部一致，旧版内容可从 Git 历史读取，不复制多套正文。


真实 API 日历及错误样例在独立环境验证，不把市场规则推断当完整性证明。许可证选择和日志轮转跨片监看方式仍是明确待决项；不得虚报完成。
